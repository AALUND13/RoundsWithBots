...
namespace RoundsWithBots.Menu {
    internal static partial class RWBMenu {
        ...
        private static void CreateStalemateMenu(GameObject mainMenu) {
            ...
            MenuHandler.CreateSlider("Stalemate Damage Cooldown", stalemateMenu, 30, 0, 60, StalemateDamageCooldown.Value, value => StalemateDamageCooldown.Value = Mathf.Round(value), out UnityEngine.UI.Slider _, true);
            AddBlank(stalemateMenu, 20);

            MenuHandler.CreateSlider("Stalemate Damage Duration", stalemateMenu, 30, 0, 60, StalemateDamageDuration.Value, value => StalemateDamageDuration.Value = Mathf.Round(value), out UnityEngine.UI.Slider _, true);
        }
    }
}
